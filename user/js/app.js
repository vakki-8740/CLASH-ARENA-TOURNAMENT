...
function shareApp() {
    if (navigator.share) {
        navigator.share({ title: 'QBIT SPORTS', text: 'Check out QBIT SPORTS - Play & Win tournaments!', url: window.location.origin + '/' }).catch(() => {});
    } else {
        navigator.clipboard.writeText(window.location.origin + '/').then(() => { showToast("Link copied!"); }).catch(() => { showToast("Share not supported", "error"); });
    }
}
...
async function saveFullProfile() {
    ...
    const btn = document.querySelector('#profile-complete-popup .pc-btn');
    btnLoading(btn, true);
    try {
        const uid = FB.getCurrentUid();
